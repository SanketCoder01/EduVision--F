...
const facultyDepartments = ['Computer Science', 'Electrical Engineering', 'Business Administration', 'Mechanical Engineering'];
...
export default function SimpleGoogleSignup({ userType, onCompleteAction, onBackAction }: SimpleGoogleSignupProps) {
  ...
  const handleGoogleAuthSuccess = (googleUser: { name: string; email: string; }) => {
    setUserData({ name: googleUser.name, email: googleUser.email });
    ...
    switch (step) {
      case 'auth': return (
        <>
          ...
          <CardContent>
            <Button onClick={() => handleGoogleAuthSuccess({ name: 'Sanket Dhabarde', email: 'sanket.dev@example.com'})} variant="outline" className="w-full h-12">
        ...
        <form onSubmit={handleDetailsSubmit}>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-1"><Label htmlFor="email">Email</Label><Input id="email" type="email" value={userData.email || ''} disabled /></div>
                        <div className="space-y-1"><Label htmlFor="department">Department</Label><Select onValueChange={val => setUserData(d => ({...d, department: val}))}><SelectTrigger><SelectValue placeholder="Select department" /></SelectTrigger><SelectContent>{(userType === 'student' ? studentDepartments : facultyDepartments).map(d => <SelectItem key={d} value={d}>{d}</SelectItem>)}</SelectContent></Select></div>
      ...
      case 'welcome': return (
        <CardContent className="text-center p-8 space-y-4">
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1}} transition={{ type: 'spring' }}><img src={userData.photo} alt="Welcome" className="w-32 h-32 rounded-full mx-auto border-4 border-green-500" /></motion.div>
          <h2 className="text-2xl font-bold">Welcome, {userData.name}!</h2>
          <p className="text-muted-foreground">Your account is ready. Redirecting...</p>
        </CardContent>
      );
    }
  }
