...
-- 9. Add password_hash column to students table if it doesn't exist
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
...
-- 10. Add password_hash column to faculty table if it doesn't exist
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
...
-- 11. Add designation column to faculty table if it doesn't exist
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
...
-- 12. Add qualification column to faculty table if it doesn't exist
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
...
-- 13. Add experience_years column to faculty table if it doesn't exist
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
...
-- 14. Update existing records to have default values where needed
UPDATE students 
SET face_data = '{}' 
...
-- 15. Show final table structure for students
SELECT 'Students table updated successfully!' as status;

-- 16. Show final table structure for faculty
