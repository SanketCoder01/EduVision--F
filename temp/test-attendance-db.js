...
async function testAttendanceDatabase() {
    ...
    try {
        ...
        const { data: cseSessions, error: cseError } = await supabase_1.supabase
            .from('attendance_sessions')
            .select('*')
            .eq('department', 'CSE')
            .eq('year', 'third');
        ...
        const { data: records, error: recordsError } = await supabase_1.supabase
        ...
        const { data: newSession, error: createError } = await supabase_1.supabase
            .from('attendance_sessions')
            .insert([testSession])
            .select()
            .single();
