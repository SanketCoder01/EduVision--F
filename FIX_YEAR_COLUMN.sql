-- ============================================
-- COMPLETE FIX FOR "column year does not exist" ERROR
-- Run this in Supabase Dashboard SQL Editor
-- ============================================

-- Step 1: Check if user_profiles table has year column
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE user_profiles ADD COLUMN year TEXT;
...
-- Step 2: Create profile_updates table with year column
...
-- Verification query - check if year column exists
SELECT 
  table_name, 
  column_name, 
  data_type 
FROM information_schema.columns 
WHERE table_name IN ('user_profiles', 'profile_updates', 'study_groups') 
...
BEGIN
  ...
  RAISE NOTICE '✅ All tables created/updated with real-time enabled';
  RAISE NOTICE '✅ Year column verified in all necessary tables';
END $$;
